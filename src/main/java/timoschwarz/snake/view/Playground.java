...
public class Playground extends JPanel
{
	...
	private ArrayList<Entity> booster;
	...
	private void drawEntitiesToScreen(Graphics2D g2d)
	{
		...
		for (int i = 0; i < snakes.size(); i++)
		{
			...
			SnakeEntity e = snakes.get(i);
			boolean isPhased = e.getSnake().isPhased();
			if (e.isVisible())
			{
				LinkedList<Rectangle2D.Double> rects = e.getRects();
				for (Rectangle2D.Double rec : rects)
				{
					if (e.getSnakeHead() == rec && isPhased)
					{
		...
		for (Entity e : booster)
		{
			graphics_buffer.setColor(Color.yellow);
			if (e.isVisible())
			{
				LinkedList<Rectangle2D.Double> rects = e.getRects();
				for (Rectangle2D.Double rec : rects)
	...
	{
		LinkedList<Piece> looseSnakePieces2 = controller.getLooseSnakePieces();
		Entity entity = EntityHelper.createEntity(looseSnakePieces2, "red");
	...
	{
		this.getBooster().clear();
		LinkedList<Piece> pieces = new LinkedList<>();
		for (Boost booster : currentBooster)
		{
			Piece piece = (Piece) booster;
			pieces.add(piece);
			Entity entity = EntityHelper.createEntity(pieces, booster.getColor());
			this.getBooster().add(entity);
		}
	}

	public ArrayList<Entity> getBooster()
	{
		return booster;
	}

	public void setBooster(ArrayList<Entity> booster)
	{
